...
class Moneymanage:
    ...
    def __init__(self):
        ...
        self.txt_total = tk.Label(self.fr_forms)
    ...
    def data_add(self):
        try:
            if self.ent_date.get() == "" or self.money.get() == "" or self.category.get() == "" or self.balance.get() != "income" and self.balance.get() != "outcome":
                self.txt_result.config(text="Fail : fill the blank", fg="red")
            else:
            ...
            self.txt_result.config(text="Fail : check the values type error", fg="red")
