...
class Moneymanage:
    ...
    def __init__(self):
        ...
        ##### TABLE #####
        # self.tree = ttk.Treeview(self.fr_table, height=500)
    ...
    def update_treeview(self):
        ...
        self.tree.column('#0', minwidth=0, width=0)
        self.tree.pack()
        self.tree['columns'] = self.df.columns.values.tolist()
    ...
    def data_add(self):
        try:
            if self.ent_date.get() == "" or self.money.get() == "" or self.balance.get() != "income" and self.balance.get() != "outcome":
                ...
                self.txt_total.config(text=f"합산:{total} | 수입:{sum_income} | 지출:{sum_outcome}", fg="blue")
    ...
    def data_delete(self):
        ...
        x = selected[0]
        self.tree.delete(x)
        self.cur.execute(f"DELETE from {self.user} where id=?", (x,))
        self.conn.commit()
        self.update_treeview()
        print(x)
        print(self.df)
